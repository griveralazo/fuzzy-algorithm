...
def get_end_activities(log, parameters=None):
    ...
    if len(log)>0:
        for trace in log:
            activity_last_event = log[trace][-1][0] #[-1]-> last activity; [0] Activity position in the list
            if activity_last_event in end_activities:
                value = end_activities[activity_last_event] + 1
            else:
                value = 1
            end_activities[activity_last_event] = value
