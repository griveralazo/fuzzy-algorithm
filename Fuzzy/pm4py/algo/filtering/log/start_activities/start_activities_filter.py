...
def get_start_activities(log, parameters=None):
    ...
    if len(log)>0:
        for trace in log:
            activity_first_event = log[trace][0][0] #[0]-> first activity; [0] Activity position in the list
            if activity_first_event in start_activities:
                value = start_activities[activity_first_event] + 1
            else:
                value = 1
            start_activities[activity_first_event] = value
