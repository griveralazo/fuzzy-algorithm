...
def freq_triple(log, parameters=None):
    if parameters is None:
        parameters = {}
    print("lilili")
    activity_key= 0
    dfgs = map((lambda t: [(t[i - 2][activity_key], t[i - 1][activity_key], t[i][activity_key]) for i in range(2, len(t))]), log)
    #print("dfgs: ", list(dfgs))

    """   
    for index, t in log.iterrows():
        for i in range(2, len(t)):
    """        

    a = Counter([dfg for lista in dfgs for dfg in lista])
    #print("counter: ", list(a))
    return a
...
def apply_heu(log, parameters=None):
    ...
    plog = preProcessLog(log)
    
    start_activities = log_sa_filter.get_start_activities(plog, parameters=parameters)
    end_activities = log_ea_filter.get_end_activities(plog, parameters=parameters)
    ...
    freq_triples = dfg_factory.apply(plog, parameters=parameters, variant="freq_triples")
    performance_dfg = dfg_factory.apply(log, parameters=parameters, variant="performance") 
#    freq_triples = dfg_factory.apply(log, parameters=parameters, variant="freq_triples")
#    freq_triples = freq_triple(log, parameters=parameters)
#    print("freq")
#    print(freq_triples)

    a = apply_heu_dfg(dfg, activities=activities, activities_occurrences=activities_occurrences,
                         start_activities=start_activities,
                         end_activities=end_activities, dfg_window_2=dfg_window_2, freq_triples=freq_triples,
                         parameters=parameters, performance_dfg=performance_dfg)
#    print(a)
    return a


def apply_heu_dfg(dfg, activities=None, activities_occurrences=None, start_activities=None, end_activities=None,
                  dfg_window_2=None, freq_triples=None, parameters=None, performance_dfg=None):
    """
    Discovers an Heuristics Net using Heuristics Miner
    ...
    heu_net = HeuristicsNet(dfg, activities=activities, activities_occurrences=activities_occurrences,
                            ...
                            performance_dfg=performance_dfg, dfg_window_2=dfg_window_2,
